...
class UI:
    def clear(title: str = None):
        if platform.system() == "Windows":
            os.system(
                f'cls {title if title != None else ""}'
            )  # clear console, change title
    ...
    def show():
        print("")
        print(
            f"{colorama.Fore.BLUE}    ____       ____  __  ____           ______         "
        )
        print(
            f"{colorama.Fore.BLUE}   / __ \_____/ __ \/ /_/ __ \____     / ____/__  ____ "
        )
        print(
            f"{colorama.Fore.BLUE}  / /_/ / ___/ / / / __/ / / / __ \   / / __/ _ \/ __ \\"
        )
        print(
            f"{colorama.Fore.BLUE} / ____/ /  / /_/ / /_/ /_/ / / / /  / /_/ /  __/ / / /"
        )
        print(
            f"{colorama.Fore.BLUE}/_/   /_/   \____/\__/\____/_/ /_/   \____/\___/_/ /_/.gg "
        )
        print(
...
class Log:
    ...
    def amazing(msg: str, symbol="U"):
        if thread_l:
            lock.acquire()
        print(
            f"[{colorama.Fore.LIGHTBLACK_EX}{datetime.now().strftime('%H:%M:%S')}{colorama.Fore.RESET}] ({colorama.Fore.BLUE}{symbol}{colorama.Fore.RESET}) - {msg}"
        )
        if thread_l:
            lock.release()
        return
    ...
    def good(msg: str, symbol="+"):
        if thread_l:
            lock.acquire()
        print(
            f"[{colorama.Fore.LIGHTBLACK_EX}{datetime.now().strftime('%H:%M:%S')}{colorama.Fore.RESET}] ({colorama.Fore.LIGHTBLUE_EX}{symbol}{colorama.Fore.RESET}) - {msg}"
        )
        if thread_l:
            lock.release()
        return
    ...
    def info(msg: str, symbol="="):
        if thread_l:
            lock.acquire()
        print(
            f"[{colorama.Fore.LIGHTBLACK_EX}{datetime.now().strftime('%H:%M:%S')}{colorama.Fore.RESET}] ({colorama.Fore.LIGHTCYAN_EX}{symbol}{colorama.Fore.RESET}) - {msg}"
        )
        if thread_l:
            lock.release()
        return
    ...
    def bad(msg: str, symbol="X"):
        if toggle_errors:
            if thread_l:
                lock.acquire()
            print(
                f"[{colorama.Fore.LIGHTBLACK_EX}{datetime.now().strftime('%H:%M:%S')}{colorama.Fore.RESET}] ({colorama.Fore.RED}{symbol}{colorama.Fore.RESET}) - {msg}"
            )
            if thread_l:
                lock.release()
        return
    ...
    def warn(msg: str, symbol="!"):
        print(
            f"[{colorama.Fore.LIGHTBLACK_EX}{datetime.now().strftime('%H:%M:%S')}{colorama.Fore.RESET}] ({colorama.Fore.YELLOW}{symbol}{colorama.Fore.RESET}) - {msg}"
        )
        return
...
class Discord:
    ...
    def __init__(self) -> None:
        ...
        self.use_hotmailbox = (
            self.data["use_hotmailbox"]
            if self.data["use_hotmailbox"] == True
            else False
        )
        self.build_num = 229256
        ...
        self.session = tls_client.Session(
            client_identifier="chrome_116", random_tls_extension_order=True
        )
        self.proxy = random.choice([prox.strip() for prox in open("proxies.txt")])
        self.x_sup = ""
        ...
        if self.data["random_username"]:
            self.username = "".join(
                random.choice(string.ascii_letters) for x in range(random.randint(4, 6))
            )
        else:
            self.username = random.choice(
                [prox.strip() for prox in open("usernames.txt", encoding="utf-8")]
            )
            ...
            try:
                ...
                pass
        if (
            self.data["verify_email"]
            and self.data["kopeechka_key"] != ""
            and len(self.data["kopeechka_domains"]) > 0
            and self.unclaimed != True
        ):
            ...
            self.email = "".join(
                random.choice(string.ascii_letters) for x in range(random.randint(4, 7))
            )
            self.email += str(
                "".join(
                    str(
                    ...
                    for x in range(int(random.randint(6, 8)))
                )
            )
        ...
        if self.data["password"] == "":
            self.password = "".join(
                ...
                for x in range(random.randint(8, 24))
            ) + "".join(
                ""
                if random.randint(1, 2) == 1
                else random.choice(["@", "$", "%", "*", "&", "^"])
        ...
        self.ua = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36"
    ...
    def display_stats():
        while True:
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                f"Pr0t0n Discord Generator | Unlocked: {unlocked} | Locked: {locked} | Unlock Rate: {ur} | Budget Left: ${round(budget, 4)} | Threads: {threading.active_count() - 2} | Time: {round(time.time() - st, 2)}s | discord.gg"
            )
            time.sleep(0.5)
    ...
    def buy_email(self):
        if self.data["kopeechka_key"] != "" and not self.use_hotmailbox:
            ...
            r = requests.get(
                f"https://api.hotmailbox.me/mail/buy?apikey={self.data['hotmailbox_key']}&mailcode=HOTMAIL&quantity=1"
            ).json()
            try:
                return (
                    r["Data"]["Emails"][0]["Email"],
                    r["Data"]["Emails"][0]["Password"],
                )
            except Exception as e:
    ...
    def check_token(self):
        ...
        if int(response.status_code) in (400, 401, 403):
            Log.bad(
                f"Locked Token ({colorama.Fore.RED}{self.token[:25]}..{colorama.Fore.RESET})"
            )
            locked += 1
            return
        else:
            Log.amazing(
                f"Unlocked Token ({colorama.Fore.LIGHTBLACK_EX}{self.token[:25]}..{colorama.Fore.RESET})"
            )
            unlocked += 1
    ...
    def ConnectWS(self):
        try:
            ...
            return
        Log.good(
            f"Onlined Token --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}..{colorama.Fore.RESET})",
            symbol="O",
        )
        return
    ...
    def get_captcha_key(
        ...
        **argv,
        ...
        serv = (
            "capmonster"
            if self.use_capmonster and self.capmonster_key != ""
            else "capsolver"
        )
        if serv != "capmonster":
            ...
            try:
                if not used_hcoop:
                    ...
                    if r.json()["status"] == "ready":
                        ...
                        Log.good(
                            f"Solved captcha ({colorama.Fore.LIGHTBLACK_EX}{cap_pri}..{colorama.Fore.RESET})"
                        )
                        return r.json()["solution"]["gRecaptchaResponse"]
                else:
                    r = requests.post(
                        f"{service}/getTaskData",
                        json={"api_key": self.hcop_key, "task_id": taskid},
                    )
    ...
    def send_msg(self):
        ...
        payload = {
            "content": str(self.data["message"]),
            "nonce": f"{Discord.nonce()}",
            "tts": False,
            "flags": 0,
        }
        try:
        ...
        if r == 200:
            Log.good(
                f"Successfullly Sent Message --> ({self.data['channel_id']})",
                symbol="MSG",
            )
        else:
            Log.bad("Error Sending Message")
    ...
    def add_hs(self):
        ...
        if st.status_code == 204:
            Log.good(
                f"Added HypeSquad -> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}..{colorama.Fore.RESET})",
                symbol="W",
            )
            return
        else:
    ...
    def buy_phonenumber(self):
        op = (
            "any"
            if len(self.data["5sim_operator"]) < 1
            else str(random.choice(self.data["5sim_operator"])).lower()
        )
    ...
    def join_server(self):
        ...
        if r.status_code == 200:
            ...
            self.session.headers["x-captcha-key"] = str(
                self.get_captcha_key(r.json()["captcha_sitekey"])
            )
        else:
            Log.bad("Error Joining Server")
        return
            ...
            if ra["value"] != None:
                try:
                    ...
                    if response.status_code == 200:
                        Log.good(
                            f"Successfully Verified Email --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}{colorama.Fore.RESET}..)",
                            "EV",
                        )
                        ...
                        if self.data["verify_email"]:
                            ...
                            payload = {
                                "token": email_token,
                                "captcha_key": str(
                                    self.get_captcha_key(
                                        response.json()["captcha_sitekey"]
                                    )
                                ),
                            }
                            try:
                                self.token = sess2.post(url, json=payload).json()[
                                    "token"
                                ]
                                Log.good(
                                    f"Verified Email with Captcha --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}{colorama.Fore.RESET}..)",
                                    "EV",
                                )
                                return
                            except:
    ...
    def add_pfp(self):
        ...
        try:
            r = self.session.patch(
                "https://discord.com/api/v9/users/@me", json=json_data
            ).json()["id"]
            Log.good(
                f"Added PFP --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}..{colorama.Fore.RESET})",
                "P",
            )
        except:
            try:
    ...
    def add_pronouns(self):
        ...
        if res.status_code == 200:
            Log.good(
                f"Added Pronoun --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}{colorama.Fore.RESET})",
                symbol="P",
            )
        else:
    ...
    def add_bio(self):
        url = "https://discord.com/api/v9/users/%40me/profile"
        payload = {"bio": str(random.choice(self.bios))}
        try:
        ...
        if r.status_code == 200:
            Log.good(
                f"Added Bio --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}..{colorama.Fore.RESET})",
                "B",
            )
        else:
            Log.bad("Error Adding Bio")
    ...
    def enable_devmode(self):
        ...
        if r_s == 200:
            Log.good(
                f"Enabled Devmode --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}..{colorama.Fore.RESET})",
                "DEV",
            )
        else:
            Log.bad("Error Enabling Devmode")
    ...
    def send_sms(self):
        ...
        if res.status_code == 204:
            ...
            if self.data["solve_phone_captcha"]:
                try:
                    self.session.headers["x-captcha-key"] = self.get_captcha_key(
                        res.json()["captcha_sitekey"]
                    )
                    res = self.session.post(url, json=json_data)
                except KeyError:
                    ...
                    if code == False:
                        ...
                        if res.status_code == 200:
                            ...
                            if res.status_code == 204 or res.status_code == 200:
                                try:
                                    Log.good(
                                        f"Successfully Verified Phone Number --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}{colorama.Fore.RESET}..)"
                                    )
                                except:
    ...
    def create_acct(self):
        ...
        if self.unclaimed:
            url = "https://discord.com/api/v9/users/@me"
            data = {
                "date_of_birth": f"{random.randint(1980, 1999)}-{random.randint(1, 10)}-{random.randint(1, 10)}"
            }
            try:
            ...
            if r == 200:
                Log.good(
                    f"Added DOB to Unclaimed --> ({colorama.Fore.LIGHTBLACK_EX}{self.token[:20]}{colorama.Fore.RESET}..)",
                    "D",
                )
            else:
                Log.bad("Error Adding DOB to unclaimed")
